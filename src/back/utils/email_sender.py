...
class EmailSender:
    ...
    @staticmethod
    def build_verification_email(username: str, activation_link: str) -> str:
        with open(EmailSender.TEMPLATE_PATH, "r", encoding="utf-8") as f:
            body = f.read()

        body = body.replace("{{ username }}", username)
        body = body.replace("{{ activation_link }}", activation_link)

        body = re.sub(r"{% if activation_link %}|{% endif %}", "", body)

        return body

    @staticmethod
    def send_email(to_email: str, username: str, activation_link: str):
        ...
        msg.attach(
            MIMEText(
                EmailSender.build_verification_email(username, activation_link), "html"
            )
        )
