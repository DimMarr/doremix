...
class EmailSender:
    ...
    TEMPLATE_VERIFICATION_PATH = os.path.join(
        os.path.dirname(__file__), "templates", "template_confirm.html"
    )
    TEMPLATE_RESET_PATH = os.path.join(
        os.path.dirname(__file__), "templates", "template_reset.html"
    )

    @staticmethod
    def build_verification_email(username: str, code: str) -> str:
        with open(EmailSender.TEMPLATE_VERIFICATION_PATH, "r", encoding="utf-8") as f:
            body = f.read()
    ...
    @staticmethod
    def build_password_reset_email(username: str, code: str) -> str:
        with open(EmailSender.TEMPLATE_RESET_PATH, "r", encoding="utf-8") as f:
            body = f.read()
        body = body.replace("{{ username }}", username)
        body = body.replace("{{ code }}", str(code))
        return body
    ...
    @staticmethod
    def send_password_reset_email(to_email: str, username: str, code: str):
        mail_server = EmailSender.MAIL_SERVER
        mail_username = EmailSender.MAIL_USERNAME
        mail_password = EmailSender.MAIL_PASSWORD
        ...
        msg["From"] = EmailSender.MAIL_FROM
        msg["To"] = to_email
        msg["Subject"] = "Réinitialise ton mot de passe Doremix 🔐"
        msg.attach(
            MIMEText(EmailSender.build_password_reset_email(username, code), "html")
        )
        ...
        try:
            ...
            server.login(mail_username, mail_password)
            server.sendmail(EmailSender.MAIL_FROM, to_email, msg.as_string())
            print(f"Password reset mail envoyé à {to_email}")
        except Exception as e:
            print(f"Erreur password reset mail à {to_email} :", e)
        finally:
            server.quit()
