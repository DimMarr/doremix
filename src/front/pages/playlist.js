...
export function renderPlaylistPage(container, playlist, trackPlayer, onBack) {
    ...
    const backButton = createButton({ textContent: '← Back', variant: 'ghost', size: 'sm' });
    ...
    const playlistTitle = createText({ textContent: playlist.name, size: '4xl', className: 'font-bold' });
    const playlistDescription = createText({ textContent: playlist.description, size: 'lg', className: 'text-muted-foreground' });
    ...
    trackListHeader.className = 'grid grid-cols-[2rem_1fr_1fr_4rem] gap-4 px-4 text-muted-foreground';
    ...
    tracks.forEach((track, index) => {
        ...
        trackRow.className = 'grid grid-cols-[2rem_1fr_1fr_4rem] items-center gap-4 px-4 py-2 rounded-md transition-colors duration-200 hover:bg-gray-800 cursor-pointer';
        ...
        playingIcon.src = new URL('../assets/icons/pause.svg', import.meta.url).href;
        playingIcon.className = 'playing-icon hidden w-4 h-4';

        trackNumberContainer.append(trackNumber, playingIcon);
        
        const trackTitle = createText({ textContent: track.title, className: 'font-medium track-title' });
        const trackArtist = createText({ textContent: (track.artist && track.artist.name) ? track.artist.name : "Unknown" });
        const trackDuration = createText({ textContent: '3:00' }); // No duration in track model yet

        trackRow.append(trackNumberContainer, trackTitle, trackArtist, trackDuration);
