...
export class ArtistRepository {
  async getAll(): Promise<Artist[]> {
    const response = await fetch(`${API_BASE_URL}/artists`, { credentials: 'include' });
  ...
  async getById(id: number): Promise<Artist> {
    const response = await fetch(`${API_BASE_URL}/artists/${id}`, { credentials: 'include' });
  ...
  async getTracks(id: number): Promise<Track[]> {
    const response = await fetch(`${API_BASE_URL}/artists/${id}/tracks`, { credentials: 'include' });
